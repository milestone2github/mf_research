...
const getRecoTransactions = async (req, res) => {
  let { amcName, schemeName, rmName, type, sort, minAmount, maxAmount} = req.query
  ...
  try {
    ...
    res.status(200).json({ 
      ...
      data: {
        page,
        totalCount: totalCountAndTotalAmount[0].totalCount,
        totalAmount: totalCountAndTotalAmount[0].totalAmount,
        transactions, 
      } 
    })
  } catch (error) {
