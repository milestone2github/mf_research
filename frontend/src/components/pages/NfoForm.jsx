...
function NfoForm() {
  ...
  async function fetchFoliosFromFolioMaster (iwellfolios, joint1, joint2) {
    try {
      ...
      const response = await fetch(`${backendUrl}/api/data/folios/from-folios/?${params}`, {
        method: 'GET',
        credentials: 'include'
      });
    ...
    const fetchIsin = () => {
      ...
      fetch(`${backendUrl}/api/data/isin?amc=${amc}`, {
        ...
        .then(jsonRes => {
          ...
          const isinFilteredFolios = foliosFromIwell.filter(folio => isinList.includes(folio.isin)).map(
            item => item.folioNo
          )
          
          fetchFoliosFromFolioMaster(isinFilteredFolios, ucc.Second_Holder_First_Name, ucc.Third_Holder_First_Name);
        })
        .catch(error => {
  ...
  const fetchFolios = async () => {
    try {
      const response = await fetch(`${backendUrl}/api/data/iwell-folios/?pan=${pan}`, {
        method: 'GET',
        credentials: 'include'
      });
      const jsonRes = await response.json();
  ...
  useEffect(() => {
    const fetchNfoData = async () => {
      ...
      try {
        const response = await fetch(`${backendUrl}/api/data/nfo-schemes?amc=${amc}&schemePlan=NORMAL&purchaseTrxMode=DP&purchaseTrxMode=P`, {
          method: 'GET',
          credentials: 'include'
        });
        const jsonRes = await response.json();
  ...
  const debouncedGetNames = useCallback(
    debounce(async (name) => {
      try {
        ...
        const response = await fetch(`${backendUrl}/api/data/investors/?name=${name}&searchall=true`, {
          method: 'GET',
          credentials: 'include'
        });
  ...
  const debouncedGetPan = useCallback(
    debounce(async (pan) => {
      try {
        ...
        const response = await fetch(`${backendUrl}/api/data/investors/?pan=${pan}&searchall=true`, {
          method: 'GET',
          credentials: 'include'
        });
        const data = await response.json();
  ...
  const debouncedGetFamilyHeads = useCallback(
    debounce(async (familyHead) => {
      try {
        ...
        const response = await fetch(`${backendUrl}/api/data/investors/?fh=${familyHead}&searchall=true`, {
          method: 'GET',
          credentials: 'include'
        });
        const data = await response.json();
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(nfoUrl)
      .then(() => {
        setShowCopied(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!nfo) {
        dispatch(updateToast({ type: 'error', message: "NFO Scheme is required" }))
        return
      }
      ...
      dispatch(updateToast({ type: 'success', message: 'Submitted successfully' }))
  ...
  if(!permissions.find(perm => perm === 'NFO')) 
    return (<AccessDenied />)
  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col gap-y-8 md:mx-0'>
      ...
      <div className='mb-2'>
        <h3 className='text-2xl w-fit font-semibold inline-block bg-clip-text text-transparent bg-gradient-to-r from-[#2D3748] to-[#1A202C]'>New Fund Offer Form</h3>
      ...
      <fieldset className='bg-gray-50 rounded-md p-6 border-indigo-200 w-auto min-w-[50vw] overflow-x-auto'>
        <UccTable data={uccList} selectedOption={ucc} updateSelected={(value) => setUcc(value)}/>
      </fieldset>
      ...
      <fieldset className='flex flex-wrap gap-8 b-purple-50 rounded-md p-6 sm:border border-green-200'>
        ...
        <div className="grow shrink basis-60 md:basis-80">
          <CustomSelect
            ...
            onSelect={(value) => {
              setNfo(value)
              setMinAmount(schemesWithCode.find(item => item.name === value).minAmount)
            }}
            options={nfoList}
          />
        </div>
        ...
        <div className="flex flex-col text-left basis-60 md:basis-80 grow shrink">
          <label htmlFor="folios" className='text-gray-800 text-sm font-medium p-0 leading-none mb-1'>Folio Number</label>
          <select
            name="folio"
            id="folios"
            value={folio}
            ...
            {folioList.map((folio, index) => (
              <option key={index} value={folio}>{folio || 'Select Folio'}</option>
            ))}
          </select>
        </div>

        <div className="grow shrink basis-60 md:basis-80 text-left flex flex-col">
          <label htmlFor="amount" className='text-gray-800 text-sm font-medium leading-none mb-1'>Amount</label>
          ...
          {nfo && <p className={`text-sm ${amount < minAmount ? 'text-red-500' : 'text-green-500'}`}>
      ...
      {nfoUrl && <fieldset className='relative flex items-center gap-x-4 b-purple-50 rounded-md p-6 sm:border bg-gray-800 text-gray-200'>
        ...
        {showCopied && <span className='absolute top-1/2 -translate-y-1/2 right-20 text-sm px-4 py-1 shadow-md shadow-gray-900 rounded-md text-green-700 border border-green-700'>
          copied
        </span>}
      </fieldset>}
